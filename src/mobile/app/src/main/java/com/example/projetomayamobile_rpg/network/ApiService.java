...
public interface ApiService {
    ...
    // TODO: substituir pelo path real    "ESQUECI SENHA"
    @POST("auth/forgot-password")   // <- PLACEHOLDER, alterar quando o backend definir
    Call<Void> forgotPassword(@Body ForgotPasswordRequest body);

    // TODO: substituir pelo path real    "ESQUECI SENHA"
    @POST("auth/verify-code")    // <- PLACEHOLDER, alterar quando o backend definir
    Call<Void> verifyCode(@Body VerifyCodeRequest body);

    // TODO: substituir pelo path real     "ESQUECI SENHA"
    @POST("auth/reset-password")    // <- PLACEHOLDER, alterar quando o backend definir
    Call<Void> resetPassword(@Body ResetPasswordRequest body);

    // TODO: substitua o path pelo endpoint real quando definido. "ALTERAR SENHA"
    @PUT("patients/change-password")    // <- PLACEHOLDER, alterar quando o backend definir
    Call<Void> changePassword(@Body ChangePasswordRequest body);

    // TODO: placeholder temporário para criar a build, apagar depois  "CHAMADA PROVISORIA PARA EVITAR ERRO"
